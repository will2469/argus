...
func TestA30_PositiveCorpus(t *testing.T) {
	...
	if base != "001_unsafe_timestamp.up.sql" {
		t.Errorf("expected issue in 001_unsafe_timestamp.up.sql, got %s", base)
	}
	if !strings.Contains(issues[0].Message, "TIMESTAMPTZ") {
		t.Errorf("expected TIMESTAMPTZ in issue message, got: %s", issues[0].Message)
	}
}

func TestA30_NegativeCorpus(t *testing.T) {
...
func TestA30_AdversarialCorpus(t *testing.T) {
	...
	assertions := []struct {
		...
		{"M5_AlterColType", "005_alter_col_type.up.sql"},
		{"M6_TimestampPrecision", "006_timestamp_precision.up.sql"},
		{"M7_MultiTimestamp", "007_multitimestamp.up.sql"},
	}
...
func TestA30_StandaloneRunner_DualPathParity(t *testing.T) {
	...
	for _, iss := range resPos.Issues {
		if iss.Rule == "ARGUS-A30" || iss.Rule == "TIMESTAMPTZ_REQUIRED" {
			posIssues++
		}
	}
	if posIssues != 1 {
