...
func parseAndInspect(t *testing.T, relPath string) ([]issueInfo, *token.FileSet) {
	...
	a25_expensive_cpu.InspectFile(file, fset, dm, func(pos token.Pos, format string, args ...any) {
...
func TestA25_PositiveCorpus(t *testing.T) {
...
func TestA25_NegativeCorpus(t *testing.T) {
...
func TestA25_AdversarialCorpus(t *testing.T) {
	...
	assertions := []struct {
		...
		{"A1_Branch", 32, true},
		{"A2_Reassignment", 40, true},
		{"A3_Alias", 54, true},
		{"A4_Wrapper", 67, true},
		{"A5_NestedFunction", 76, true},
		{"A6_Generic", 90, true},
		{"A7_ECDSAKeygen", 98, true},
	}
...
func TestA25_StandaloneRunner_DualPathParity(t *testing.T) {
	...
	for _, iss := range res.Issues {
		if iss.Rule == "EXPENSIVE_CPU_IN_TX" || iss.Rule == "ARGUS-A25" {
			a25Issues++
		}
	}
	if a25Issues == 0 {
