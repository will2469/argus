...
func P1_Obvious(ctx context.Context, db DBExecutor, id string) {
	_, _ = db.Query(ctx, "SELECT * FROM users WHERE id = "+id) // want `\[ARGUS-A01\] unsafe SQL concatenation or formatting`
}
...
func P2_Indirect(ctx context.Context, db DBExecutor, status string) {
	...
	query += status + "'"
	_, _ = db.Query(ctx, query) // want `\[ARGUS-A01\] unsafe SQL concatenation or formatting`
}
...
func executeUserDeleteHelper(ctx context.Context, db DBExecutor, id string) {
	...
	_, _ = db.Exec(ctx, sql) // want `\[ARGUS-A01\] unsafe SQL concatenation or formatting`
}
...
func P4_Nested(ctx context.Context, db DBExecutor, roles []string) {
	for _, role := range roles {
		if role != "" {
			_ = db.QueryRow(ctx, "SELECT count(*) FROM members WHERE role = '"+role+"'") // want `\[ARGUS-A01\] unsafe SQL concatenation or formatting`
		}
	}
}
...
func P5_Alias(ctx context.Context, r AppDB, id string) {
	_, _ = r.Query(ctx, "SELECT * FROM accounts a WHERE a.id = "+id) // want `\[ARGUS-A01\] unsafe SQL concatenation or formatting`
}
...
func P_Ignored(ctx context.Context, db DBExecutor, id string) {
	...
	_, _ = db.Exec(ctx, "DELETE FROM temp_sessions WHERE id = "+id)
}
