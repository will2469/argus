...
func parseAndInspect(t *testing.T, relPath string) ([]issueInfo, *token.FileSet) {
	...
	a23_tx_timeout.InspectFile(file, fset, dm, func(pos token.Pos, format string, args ...any) {
...
func TestA23_PositiveCorpus(t *testing.T) {
...
func TestA23_NegativeCorpus(t *testing.T) {
...
func TestA23_AdversarialCorpus(t *testing.T) {
	...
	assertions := []struct {
		...
		{"A3_Alias", 30, true},
		{"A4_Wrapper", 42, true},
		{"A5_NestedFunction", 48, true},
		{"A6_Generic", 57, true},
		{"A7_ExplicitZero", 62, true},
	}
...
func TestA23_StandaloneRunner_DualPathParity(t *testing.T) {
	...
	for _, iss := range res.Issues {
		if iss.Rule == "MISSING_TX_TIMEOUT" || iss.Rule == "ARGUS-A23" {
			a23Issues++
		}
	}
	if a23Issues == 0 {
