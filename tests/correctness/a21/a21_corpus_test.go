...
func parseAndInspect(t *testing.T, relPath string) ([]issueInfo, *token.FileSet) {
	t.Helper()
	...
	keyColumnMap := a21_row_lock.GetKeyColumns(nil)

	var issues []issueInfo
	a21_row_lock.InspectFile(file, fset, dm, keyColumnMap, func(pos token.Pos, format string, args ...any) {
...
func TestA21_PositiveCorpus(t *testing.T) {
...
func TestA21_NegativeCorpus(t *testing.T) {
...
func TestA21_AdversarialCorpus(t *testing.T) {
...
func TestA21_StandaloneRunner_DualPathParity(t *testing.T) {
	...
	for _, iss := range res.Issues {
		if iss.Rule == "BLOCKING_ROW_LOCK" || iss.Rule == "ARGUS-A21" {
			a21Issues++
		}
	}
	if a21Issues == 0 {
