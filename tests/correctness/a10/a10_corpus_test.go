...
func parseAndInspect(t *testing.T, relPath string) ([]a10_isolation_level.Issue, *token.FileSet) {
	t.Helper()
	...
	issues := a10_isolation_level.InspectFile(nil, fset, file, dm, nil)
	return issues, fset
}

func TestA10_PositiveCorpus(t *testing.T) {
...
func TestA10_NegativeCorpus(t *testing.T) {
...
func TestA10_AdversarialCorpus(t *testing.T) {
...
func TestA10_StandaloneRunner_DualPathParity(t *testing.T) {
	...
	for _, iss := range res.Issues {
		if iss.Rule == "WEAK_ISOLATION_LEVEL" {
			a10Issues++
		}
	}
	if a10Issues != 7 {
		t.Errorf("Dual-Path Parity FAILED on positive corpus: expected 7 issues from standalone runner, got %d", a10Issues)
	}
