...
func parseAndInspect(t *testing.T, relPath string) ([]a03_context.Issue, *token.FileSet) {
	t.Helper()
	...
	issues := a03_context.InspectFile(nil, fset, file, dm)
	return issues, fset
}

func TestA03_PositiveCorpus(t *testing.T) {
...
func TestA03_NegativeCorpus(t *testing.T) {
...
func TestA03_AdversarialCorpus(t *testing.T) {
...
func TestA03_StandaloneRunner_DualPathParity(t *testing.T) {
	...
	for _, iss := range res.Issues {
		if iss.Rule == "UNBOUNDED_CONTEXT" {
			a03Issues++
		}
	}
	if a03Issues != 5 {
		t.Errorf("Dual-Path Parity FAILED on positive corpus: expected 5 issues from standalone runner, got %d", a03Issues)
	}
