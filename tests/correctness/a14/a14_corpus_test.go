...
func parseAndInspect(t *testing.T, relPath string) ([]a14_select_star.Issue, *token.FileSet) {
	t.Helper()
	...
	issues := a14_select_star.InspectFile(nil, fset, file, dm)
	return issues, fset
}

func TestA14_PositiveCorpus(t *testing.T) {
...
func TestA14_NegativeCorpus(t *testing.T) {
...
func TestA14_AdversarialCorpus(t *testing.T) {
...
func TestA14_StandaloneRunner_DualPathParity(t *testing.T) {
	...
	for _, iss := range res.Issues {
		if iss.Rule == "FORBIDDEN_SELECT_STAR" || iss.Rule == "ARGUS-A14" {
			a14Issues++
		}
	}
	if a14Issues == 0 {
