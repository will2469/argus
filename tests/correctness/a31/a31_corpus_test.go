...
func parseAndInspect(t *testing.T, relPath string) ([]a31_mutation_audit_trail.Issue, *token.FileSet) {
	t.Helper()
	...
	issues := a31_mutation_audit_trail.InspectFile(nil, fset, file, dm, auditMethods, exemptTables)
	return issues, fset
}

func TestA31_PositiveCorpus(t *testing.T) {
	...
	expectedLines := map[int]string{
		17: "P1_Obvious",
		25: "P2_InsertWithoutAudit",
		33: "P3_DeleteWithoutAudit",
		46: "P4_Interprocedural",
	}
...
func TestA31_NegativeCorpus(t *testing.T) {
...
func TestA31_AdversarialCorpus(t *testing.T) {
	...
	assertions := []struct {
		...
		{"A1_MultipleMutationsWithSingleAudit", 22, false},
		{"A2_AuditInHelper", 34, false},
		{"A3_ConditionalMutationWithoutAudit", 52, true},
		{"A4_AuditBeforeMutation", 60, false},
		{"A5_ExplicitTxWithoutAudit", 76, true},
		{"A6_ExplicitTxWithAudit", 88, false},
	}
	...
	for _, a := range assertions {
		...
		if a.mustDetect && !detected {
			...
			t.Logf("  [RESILIENT] %s -> PASS", a.vector)
		}
	}
}

func TestA31_StandaloneRunner_DualPathParity(t *testing.T) {
	...
	for _, iss := range res.Issues {
		if iss.Rule == "UNGUARDED_MUTATION_WITHOUT_AUDIT" || iss.Rule == "ARGUS-A31" {
			a31Issues++
		}
	}
	if a31Issues != 4 {
		t.Errorf("Dual-Path Parity FAILED on positive corpus: expected 4 issues from standalone runner, got %d", a31Issues)
	}
